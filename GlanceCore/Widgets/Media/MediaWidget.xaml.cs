...
{
    // Win32 For Background Capture
    [DllImport("gdi32.dll")] static extern bool BitBlt(IntPtr hdcDest, int xDest, int yDest, int wDest, int hDest, IntPtr hdcSource, int xSrc, int ySrc, int rop); [DllImport("user32.dll")] static extern IntPtr GetDesktopWindow(); [DllImport("user32.dll")] static extern IntPtr GetWindowDC(IntPtr hWnd);
    [DllImport("user32.dll")] static extern int ReleaseDC(IntPtr hWnd, IntPtr hDC); [DllImport("gdi32.dll")] static extern IntPtr CreateCompatibleDC(IntPtr hDC); [DllImport("gdi32.dll")] static extern IntPtr CreateCompatibleBitmap(IntPtr hDC, int nWidth, int nHeight);
    ...
    public MediaWidget()
    {
        ...
        _captureTimer = new DispatcherTimer(DispatcherPriority.Render) { Interval = TimeSpan.FromMilliseconds(50) };
    ...
    private const uint WDA_EXCLUDEFROMCAPTURE = 0x00000011; // Defined here for MediaWidget
    ...
    // English: Update Timeline properties correctly in TimelineTimer_Tick
    private void TimelineTimer_Tick(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (timeline != null)
            {
                Dispatcher.Invoke(() =>
                {
                    // English: Ensure values are white-readable and not 0/0
